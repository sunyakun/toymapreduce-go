...
rm -rf output/mr-tmp
mkdir -p output/mr-tmp/input || exit 1
cd output/mr-tmp || exit 1
rm -f mr-*
...
$TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/wc.so &
$TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/wc.so &
$TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/wc.so &
...
# start multiple workers
$TIMEOUT $BINARY_DIR/mrworker $BINARY_DIR/indexer.so
$TIMEOUT $BINARY_DIR/mrworker $BINARY_DIR/indexer.so &
...
# start multiple workers.
($TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/early_exit.so ; touch $DF) &
($TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/early_exit.so ; touch $DF) &
($TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/early_exit.so ; touch $DF) &
...
# start multiple workers
$TIMEOUT $BINARY_DIR/mapreduce worker $BINARY_DIR/crash.so &
